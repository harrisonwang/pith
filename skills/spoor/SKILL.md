...
| `slide_no_text_layer` | 该幻灯片"什么都没拿到"（正文零文本，内容全在图像/图形里），比 `embedded_visuals_omitted` 更强；必须按占位符 `--extract` 取图交 VLM，否则视该页信息缺失。消息会注明演讲者备注是否已提取 |
| `hidden_slide_omitted` | 该幻灯片被作者隐藏，正文已省略但页码保留；引用其它页时编号不受影响，不要把此页当作空白页 |
